...
//
import { CollectionsGet, CollectionsCreate, ItemTemplate, ItemTypeCreatorTypes, ItemTypeFields } from '../../src/js/actions/';
import { collectionsRawResponse, itemTemplateResponse, itemTypeCreatorTypesResponse, itemTypeFieldsResponse  } from '../fixtures';
...
describe('Web Library: Reducers', () => {
	...
	describe('Handle meta requests', () => {
		it('should handle get item template response', () => {
			state = reduce(state, ItemTemplate.response(
				makeMockResponse(itemTemplateResponse), 'document')
			);
			assert.strictEqual(state.fetching.meta, false);
			assert.isObject(state.meta.itemTemplates.document);
			assert.containsAllKeys(state.meta.itemTemplates.document, ['itemType', 'title', 'creators', 'tags']);
			assert.strictEqual(state.meta.itemTemplates.document.itemType, 'document');
		});

		it('should handle get item type creator types response', () => {
			state = reduce(state, ItemTypeCreatorTypes.response(
				makeMockResponse(itemTypeCreatorTypesResponse), 'document')
			);
			assert.strictEqual(state.fetching.meta, false);
			assert.isArray(state.meta.itemTypeCreatorTypes.document);
			assert.hasAllKeys(state.meta.itemTypeCreatorTypes.document[0], ['creatorType', 'localized']);
		});

		it('should handle get item type fields response', () => {
			state = reduce(state, ItemTypeFields.response(
				makeMockResponse(itemTypeFieldsResponse), 'document')
			);
			assert.strictEqual(state.fetching.meta, false);
			assert.isArray(state.meta.itemTypeFields.document);
			assert.hasAllKeys(state.meta.itemTypeFields.document[0], ['field', 'localized']);
		});
	});

	describe('Handle collection requests', () => {
		it('should handle get collection response', () => {
			state = reduce(state, CollectionsGet.response(
				makeMockResponse(collectionsRawResponse), 'u1')
			);
			assert.isObject(state.libraries.u1.collections);
			assert.lengthOf(Object.keys(state.libraries.u1.collections), 4);
			assert.containsAllKeys(state.libraries.u1.collections.BBBBBBBB, ['key', 'version', 'data', 'library']);
		});

		it('should handle post collections response', () => {
			state = reduce(state, CollectionsCreate.response(
				makeMockResponse(collectionsRawResponse), { payload: 'tests' }, 'u1')
			);
			assert.isObject(state.libraries.u1.collections);
			assert.lengthOf(Object.keys(state.libraries.u1.collections), 4);
			assert.containsAllKeys(state.libraries.u1.collections.BBBBBBBB, ['key', 'version', 'data', 'library']);
...
// 	it('should process actions related to fetching collections', () => {
...
// 	it('should handle errors while fetching collections', () => {
// 		var state = {};
...
// 	it('should process actions related to fetching items', () => {
// 		var state = {
// 			meta: {
// 				itemTypes: [
