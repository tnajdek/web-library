...
const ZoteroStreamingClient = () => {
	...
	const retryMs = useRef(null);
	const retryTimeout = useRef(null);
	const retryIntervalIndex = useRef(0);

	const closeUnconfirmedSubscription = useCallback(() => {
		console.warn(`Failed to confirm subscription creation within ${retryMs.current / 1000} seconds. Closing connection.`);
		ws.current.close();
	}, []);
	...
	const createSubscriptions = useCallback(() => {
		...
		if (retryMs.current) {
			retryTimeout.current = setTimeout(closeUnconfirmedSubscription, retryMs.current);
		}
	...
	const connect = useCallback(() => {
		try {
			...
			console.error(`Failed to estabilish connection to the streaming API. Retrying in ${intervals[retryIntervalIndex.current] / 1000} seconds`);
			setTimeout(connect, intervals[retryIntervalIndex.current]);
			retryIntervalIndex.current = Math.min(retryIntervalIndex.current + 1, intervals.length - 1);
		}
